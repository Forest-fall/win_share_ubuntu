...
training_epochs = 20000   # training epochs
...
weights = {
    ...
    'decoder_h3': tf.get_variable(name='decoder_h3', shape=[n_hidden_1, n_input], initializer=tf.random_normal_initializer),
}
...
biases = {
    ...
    'decoder_b3': tf.get_variable(name='decoder_b3', shape=[n_input], initializer=tf.random_normal_initializer),
}
...
W, H = model.fit_transform(train_image, r_components = t, initW=False, givenW=0)
...
cost2 = 0.5 * lambda1 * tf.norm(y_pred - tf.matmul(encoder_op, H ))/n_input
